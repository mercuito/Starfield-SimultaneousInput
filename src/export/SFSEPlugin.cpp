...
extern "C" DLLEXPORT bool SFSEAPI SFSEPlugin_Load(const SFSE::LoadInterface* a_sfse)
{
	...
	// The original plugin only NOPed the first occurrence, leaving the right
	// stick free to flip "active device = gamepad" — which then leaked into
	// downstream look-sensitivity scaling and made mouse moves use gamepad-
	// scale sensitivity until WASD movement reset the active-device state.
	// We now NOP every occurrence within the scan window.
	...
	try {
		...
		for (std::size_t i = 0; i + 4 <= kScanLimit; ++i) {
			if (p[i] == 0xC6 && p[i + 1] == 0x43 && p[i + 2] == 0x08 && p[i + 3] == 0x01) {
				...
				++patched;
				i += 3;  // skip past this match; matches don't overlap
			}
		}
		if (patched > 0) {
			++g_hooksInstalled;
